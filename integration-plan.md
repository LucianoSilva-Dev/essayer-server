...
As Fases 1 e 3 são **independentes** e podem ser feitas em paralelo. O Anglo só precisa do contrato de headers e dos schemas de request/response — que já existem e não mudam com a autenticação por API key. A Fase 2 (testes E2E) depende da Fase 1.

```
Fase 1 (essayer) ──► Fase 2 (essayer)
Fase 3 (anglo)   ──────────────────────► Integração final
```

| Ordem | Tarefa | Projeto | Duração Estimada | Deps |
|-------|--------|---------|-------------------|------|
| 1a | 1.1 Instalar plugin + Config auth.ts | essayer | 30min | - |
| 1b | 1.2 Schema + Migration (IntegrationUser) | essayer | 30min | - |
| 1c | 1.3 Core Integration Module (middleware + guard + service) | essayer | 2-3h | 1.1, 1.2 |
| 1d | 1.4 Seed Script | essayer | 30min | 1.3 |
| 3a | 3.1 IncitaClient Module | anglo | 2h | - |
| 3b | 3.2 User Sync | anglo | 2h | 3.1 |
| 3c | 3.3 Proxy Leitura | anglo | 2h | 3.1 |
| 3d | 3.4 Proxy Criação | anglo | 2h | 3.3 |
| 2 | 2.1-2.2 Testes E2E | essayer | 2-3h | 1.4 |

**Total estimado:** ~13-16h de trabalho (3h a menos que o plano original com guard customizado)

**Em paralelo:** Fases 1+3 juntas ~10h → tempo real ~10h (vs 13-16h sequencial)
...
- [ ] 3.2 - Sincronização de Usuários (lazy para alunos)
- [ ] 3.3 - Proxy Repertórios (Leitura + Interação)
- [ ] 3.4 - Proxy Repertórios (Criação)
- [ ] 3.5 - E2E Tests (com mock do Incita)

### Fase 4: Futuro (não implementado nesta iteração)

- [ ] 4.1 - `POST /api/admin/professors` no anglo-server com sincronização eager ao Incita
- [ ] 4.2 - E2E tests contra Incita real (não mockado)
- [ ] 4.3 - Cache/Redis para responses do Incita (GET repertórios)
- [ ] 4.4 - Controle granular de permissões por API key (V2 - scopes por endpoint)
- [ ] 4.5 - Webhook do Incita para atualizar `incitaVerified` no anglo-server (`POST /api/webhooks/incita/professor-verified`)
- [ ] 4.6 - Geração automática do IncitaClient via Orval a partir do OpenAPI spec (avaliar)
