...
export class ActivityController {
  ...
  start(@Param('id') activityId: string, @Session() session: UserSession) {
  ...
  updateFeedbackStatus(@Param('id') responseId: string, @Session() session: UserSession) {
    return this.service.updateFeedbackStatus(responseId, session.user.id);
  }
  ...
  getCorrection(
    ...
    return this.service.getCorrection(activityId, studentId, session.user.id);
  }
  ...
  create(@Body() data: CreateEssayActivityDto, @Session() session: UserSession) {
  ...
  getAllAnswers(
    ...
    return this.service.getAllAnswers(activityId, session.user.id, query.offset, query.limit);
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Returns the four most recent activities from all classes created by the teacher.',
  })
  @ZodResponse({
    status: 200,
  ...
  delete(@Param('id') activityId: string, @Session() session: UserSession) {
