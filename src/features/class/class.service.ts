...
export class ClassService {
  ...
  async getAll(query: GetAllClassesQueryDto, userId: string) {
    try {
      ...
      const pages = Array.from(
        { length: totalPages },
        (_, i) => `offset=${i * query.limit}&limit=${query.limit}`,
      );
      ...
      return {
        ...
        pagination: {
          ...
          limit: query.limit,
          nextPageUrl:
            nextOffset >= totalDocuments ? null : `/offset=${nextOffset}&limit=${query.limit}`,
  ...
  async getAllActivities(classId: string, userId: string, query: GetAllActivitiesQueryDto) {
    try {
      ...
      return activities.map((activity) => ({
        ...
        status: this.calculateActivityStatus(
          activity.deadline,
          activity.essay?.responses?.[0]?.answerDate,
        ),
      }));
    } catch (err) {
      console.log(err);
  ...
  async getAllActivitiesCreator(classId: string, userId: string, query: GetAllActivitiesQueryDto) {
    try {
      ...
      return activities.map((activity) => ({
        ...
        activityType: activity.type,
        usersResponded:
          activity.essay?.responses?.filter((r) => r.answerDate).map((r) => r.student) || [],
  ...
  private calculateActivityStatus(
    deadline: Date | null,
    answerDate: Date | null | undefined,
  ): string {
    if (answerDate) {
      return 'Completed';
    }
